...
class AgentsBuilderThought(AbstractThought):
    def __init__(
        self,
        openai_api_key: str,
    ...
    def run(self, category: NewsCategory, country: str):
        def split_list_into_parts(data_list, num_parts):
            # Calculate the size of each part
            avg_size = len(data_list) // num_parts
        ...
        split_data = split_list_into_parts(level3_data, 3)
        print(split_data)
...
if __name__ == "__main__":
    ...
    agent_builder = AgentsBuilderThought(openai_api_key=openai_api_key)
