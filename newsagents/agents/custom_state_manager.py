...
from newsagents.agents.memories.custom_simulation_memory import SimulationMemory
from newsagents.world.custom_actions import AgentWantsUpdatedStateEvent

class CustomStateManager(AbstractStateManager):
    ...
    def get_updated_state(self) -> AbstractAgent:
        event = AgentWantsUpdatedStateEvent(
                sender_id=self.host_agent.id, target_id=self.host_agent.host_world_id
            )
        self.host_agent.send_event(event)
        self.memory.add_event(event.model_dump_json(), summarize=True)
